package Blocks.Teleporter{
	...
	public class GreenTeleporter extends BlockBase{
		...
		public function GreenTeleporter(inputtedColumn, inputtedRow, inputtedID):void{ //Sets up the yellow plate's visuals 
		...
		override public function BlockUpdate(updater, updateType):void{ //Checks if the gate should open or attempt to close
			...
			if(updateType == "BlockEnter" && ((Math.abs(updater.row - row) == 1 && Math.abs(updater.column - column) == 0) || (Math.abs(updater.row - row) == 0 && Math.abs(updater.column - column) == 1)) && TileDatabase.GetTile(updater.column, updater.row).occupiedBlock == updater && updater.specialTags.indexOf("Immovable") == -1 && expectedTeleport.indexOf(updater) == -1 && (TileDatabase.GetTile(updater.column, updater.row).subOccupiedBlock == null || GreenTeleporter != getDefinitionByName(getQualifiedClassName(TileDatabase.GetTile(updater.column, updater.row).subOccupiedBlock)) || TileDatabase.GetTile(updater.column, updater.row).subOccupiedBlock.power == 0)){
