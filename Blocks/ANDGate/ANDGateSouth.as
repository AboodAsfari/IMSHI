package Blocks.ANDGate{
	...
	public class ANDGateSouth extends BlockBase{
		...
		public function ANDGateSouth(inputtedColumn, inputtedRow, inputtedID):void{ //Sets up the gate visuals 
		...
		public function TransmitSignal(strength, signalSource, sourceDirection):void{ //Transmits a power signal
			if(sourceDirection[1] != -1){
				...
				if((westInput != 0 || !visual.connector.wireLeft.visible) && (northInput != 0 || !visual.connector.wireTop.visible) && (eastInput != 0 || !visual.connector.wireRight.visible)){
					if(power == 0){
						power++;
