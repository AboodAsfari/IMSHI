package Blocks.ANDGate{
	import flash.utils.getQualifiedClassName;
	import flash.utils.getDefinitionByName;
	import flash.events.Event;
	
	public class ANDGateEast extends BlockBase{
		...
		public var northInput:Number;
		public var southInput:Number;
		public var westInput:Number;
		
		public function ANDGateEast(inputtedColumn, inputtedRow, inputtedID):void{ //Sets up the gate visuals 
		...
		public function TransmitSignal(strength, signalSource, sourceDirection):void{ //Transmits a power signal
			if(sourceDirection[0] != -1){
				...
				if(sourceDirection[0] == 1){
					...
					northInput += strength;
					if((northInput == 1 && strength == 1) || (northInput == 0 && strength == -1)){
				...
				var target = TileDatabase.GetTile(column + 1, row); //See above
				if((westInput != 0 || !visual.connector.wireLeft.visible) && (northInput != 0 || !visual.connector.wireTop.visible) && (southInput != 0 || !visual.connector.wireBottom.visible)){
					if(power == 0){
						power++;
						if(target != null && target.subOccupiedBlock != null && target.subOccupiedBlock.specialTags.indexOf("Receiver") != -1){
							target.subOccupiedBlock.TransmitSignal(1, this, [1, 0]);
						}
