package Blocks.Piston{
	...
	public class WestPiston extends BlockBase{	
		...
		public function WestPiston(inputtedColumn, inputtedRow, inputtedID):void{ //Sets up the yellow plate's visuals 
		...
		private function PistonAnimator(e:Event){
			if(power > 0 && (visualState == "Off" || (visualState == "On" && (visual.currentFrame > 8 || visual.currentFrame == 1)))){
				var tile:Object = TileDatabase.GetTile(column - 1, row);
				if(tile != null && (tile.occupiedBlock == null || (tile.occupiedBlock.specialTags.indexOf("Immovable") == -1 && TileDatabase.GetTile(column - 2, row) != null && TileDatabase.GetTile(column - 2, row).occupiedBlock == null))){
					...
					if(tile.occupiedBlock != null){
						tile.occupiedBlock.Move(-1, 0, true);
					}
				...
				var retractionTile:Object = TileDatabase.GetTile(column - 1, row);
