package Blocks.Observer{
	...
	public class SouthObserver extends BlockBase{
		...
		public function SouthObserver(inputtedColumn, inputtedRow, inputtedID):void{ //Sets up the observer visuals 
		...
		override public function BlockUpdate(updater, updateType):void{ //Checks if the block being observed updated
			if(isInit && updater.column == column && updater.row == row + 1){
				if(power == 0){
					power++;
					
					var target:Object = TileDatabase.GetTile(column, row - 1); //See above
