package BlockVisuals.Wire{
	...
	public class RedWireVisual extends MovieClip{
		public var script:Object;
		public const specialTags = ["Receiver", "Transmitter", "Wire", "Red"];
		
		public function RedWireVisual(inputtedScript):void{ //Sets the block coordinates
			...
			this.wireRight.gotoAndStop(5);
			this.wireLeft.gotoAndStop(5);
			this.wireTop.gotoAndStop(5);
			this.wireBottom.gotoAndStop(5);
			...
			for(var a:Number = 0; a < neighboringTiles.length; a++){ //See above
				if(!Main.stageRef.contains(Main.mapEditorUI)){ //See above
					...
					if(currentBlock != null || currentSubBlock != null){ //See above
						if((currentSubBlock != null && (currentSubBlock.specialTags.indexOf("Wire") == -1 || currentSubBlock.specialTags.indexOf("Red") != -1) && (currentSubBlock.specialTags.indexOf("Receiver") != -1 || currentSubBlock.specialTags.indexOf("Transmitter") != -1)) || (currentBlock != null && (currentBlock.specialTags.indexOf("Wire") == -1 || currentBlock.specialTags.indexOf("Red") != -1) && (currentBlock.specialTags.indexOf("Receiver") != -1 || currentBlock.specialTags.indexOf("Transmitter") != -1))){
							neighboringWires.push(neighboringTiles[a]); //See above
		...
		public function RequestConnection(requester, column, row, connectionRole):Boolean{
			if(requester.specialTags.indexOf("Wire") == -1 || requester.specialTags.indexOf("Red") != -1){
				return true;
			}else{
				return false;
			}
