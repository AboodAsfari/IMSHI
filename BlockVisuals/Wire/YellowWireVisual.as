package BlockVisuals.Wire{
	...
	public class YellowWireVisual extends MovieClip{
		public var script:Object;
		public const specialTags = ["Receiver", "Transmitter", "Wire", "Yellow"];
		
		public function YellowWireVisual(inputtedScript):void{ //Sets the block coordinates
			...
			for(var a:Number = 0; a < neighboringTiles.length; a++){ //See above
				if(!Main.stageRef.contains(Main.mapEditorUI)){ //See above
					...
					if(currentBlock != null || currentSubBlock != null){ //See above
						if((currentSubBlock != null && (currentSubBlock.specialTags.indexOf("Wire") == -1 || currentSubBlock.specialTags.indexOf("Yellow") != -1) && (currentSubBlock.specialTags.indexOf("Receiver") != -1 || currentSubBlock.specialTags.indexOf("Transmitter") != -1)) || (currentBlock != null && (currentBlock.specialTags.indexOf("Wire") == -1 || currentBlock.specialTags.indexOf("Yellow") != -1) && (currentBlock.specialTags.indexOf("Receiver") != -1 || currentBlock.specialTags.indexOf("Transmitter") != -1))){
							neighboringWires.push(neighboringTiles[a]); //See above
		...
		public function RequestConnection(requester, column, row, connectionRole):Boolean{
			if(requester.specialTags.indexOf("Wire") == -1 || requester.specialTags.indexOf("Yellow") != -1){
				return true;
			}else{
				return false;
			}
