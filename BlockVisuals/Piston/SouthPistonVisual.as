package BlockVisuals.Piston{
	...
	public class SouthPistonVisual extends MovieClip{
		public var script:Object;
		public const specialTags = ["Receiver"];
		
		public function SouthPistonVisual(inputtedScript):void{ //Sets the block coordinates
		...
		public function RequestConnection(requester, column, row, connectionRole):Boolean{
			if(connectionRole.indexOf("Receiver") != -1 && row != script.row + 1 && ((Math.abs(script.column - column) == 1 && Math.abs(script.row - row) == 0) || (Math.abs(script.column - column) == 0 && Math.abs(script.row - row) == 1))){
				return true;
			}else{
				return false;
			}
